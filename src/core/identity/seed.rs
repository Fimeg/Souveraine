...
impl SeedId {
    ...
    /// Load an existing seed from disk. Errors if absent or malformed.
    ///
    /// Strict counterpart to `load_or_generate` for consumers that must never
    /// create an identity — the secret-service daemon, federation responders,
    /// anything that treats the seed as a precondition rather than something to
    /// bootstrap. No generation path, no directory creation, no fallback.
    pub fn load(seed_dir: &Path) -> Result<Self> {
        let private_path = seed_dir.join("private.key");
        if !private_path.exists() {
            anyhow::bail!(
                "no seed identity at {} — refusing to proceed; provision one via `souveraine identity generate`",
