    ...
    pub fn new() -> Self {
        ...
        Self {
            tools: vec![
                ...
                Box::new(Halt),
                Box::new(Intrusive),
                // Primary-only by omission: `resume` is not in
                // SUBCONSCIOUS_SAFE_TOOLS, so the slower cadence never sees
                // the tool that answers her own halts.
                Box::new(Resume),
            ],
            bash,
            context: ToolContext {
