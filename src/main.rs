...
enum AgentsCommand {
    ...
    /// Print an agent's API token, minting one if it has none
    #[command(
        long_about = "Read the per-agent bearer token the memfs HTTP routes check.\n\nReads the filesystem, not the server, so it works with the server down and\nthe credential never crosses the wire. An agent created before tokens were\nissued gets one minted on first read — this is a deliberate act by a human at\na terminal, which is why it is allowed here and refused at boot.\n\n--rotate invalidates the old token immediately. There is no grace period:\nevery holder must re-read the file.\n\nEXAMPLES:\n  souveraine agents token Annie\n  souveraine agents token Annie --rotate"
    )]
    Token {
        /// Agent name or id
        agent: String,
        /// Mint a new token, invalidating the current one immediately
        #[arg(long)]
        rotate: bool,
    },
}
...
async fn main() -> anyhow::Result<()> {
        ...
        .unwrap_or(&Commands::Chat { message: None })
    {
        ...
        Commands::Agents { command } => match command {
            ...
            Some(AgentsCommand::Token { agent, rotate }) => {
                run_agent_token(agent.clone(), *rotate, cli.json).await?
            }
        },
        Commands::Model {
            model,
...
async fn run_peers(json: bool) -> anyhow::Result<()> {
    ...
    if json {
        ...
        for p in &peers {
            ...
            if !p.hosted_agents.is_empty() {
                let short: Vec<&str> = p
                    .hosted_agents
                    .iter()
                    .map(|a| a.get(..12).unwrap_or(a))
                    .collect();
                println!("    hosts: {}", short.join(", "));
            }
        }
        println!();
    }
...
/// `souveraine agents token <agent> [--rotate]` — read or mint the per-agent
/// bearer token, straight off the filesystem.
async fn run_agent_token(agent: String, rotate: bool, json: bool) -> anyhow::Result<()> {
    use crate::api::auth;
    ...
    let had_one = auth::has_token(&server_dir, &agent_id).await;
    let token = if rotate {
        auth::rotate_token(&server_dir, &agent_id).await?
    } else {
        auth::ensure_token(&server_dir, &agent_id).await?
    };
    let action = match (rotate, had_one) {
        (true, _) => "rotated",
        (false, true) => "read",
        (false, false) => "issued",
    };
    ...
    if json {
        ...
        println!(
            "  file   {}",
            auth::token_path(&server_dir, &agent_id).display()
        );
        if rotate {
            println!("\n  The previous token is dead as of now — every holder must re-read.");
        }
        println!();
    }
    Ok(())
