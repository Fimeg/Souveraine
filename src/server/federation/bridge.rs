...
pub struct FederationBridge {
    ...
    role: FederationRole,
    /// `~/.souveraine` — announces read hosted-agent seeds beneath it.
    ...
    loop {
        match tokio_tungstenite::connect_async(endpoint.as_str()).await {
            Ok((mut ws, _resp)) => {
                ...
                // Announce our presence to the peer. Hosted agents are read
                // fresh on every (re)connect — agents created since the last
                // announce are picked up on the next reconnect cycle.
                let announce = SensorEvent {
                    ...
                    payload: Some(serde_json::json!({
                        ...
                        "role": role.as_str(),
                        "hosted_agents": super::load_hosted_agent_pubkeys(&base),
                    })),
                    seed_id: None,
                    reply_to: None,
                };
