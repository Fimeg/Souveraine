use super::{portrait_cover_crop, App};
...
impl App {
    pub(super) fn load_image_protocol(&mut self, path: &std::path::Path) {
        ...
        let dyn_img = match image::ImageReader::open(path) {
            Ok(reader) => match reader.decode() {
                Ok(img) => img,
                Err(e) => {
                    tracing::warn!(path = %path.display(), error = %e, "image protocol decode failed");
                    return;
                }
            },
            Err(e) => {
                tracing::warn!(path = %path.display(), error = %e, "image protocol open failed");
                return;
            }
        };
        ...
        match picker.new_protocol(
            ...
            Err(e) => {
                tracing::warn!(path = %path.display(), error = %e, "image protocol creation failed")
            }
        }
    }
    ...
    pub(super) fn load_card_image(&mut self, agent_id: &str, path: &std::path::Path) {
        ...
        let dyn_img = match image::ImageReader::open(path) {
            Ok(reader) => match reader.decode() {
                Ok(img) => img,
                Err(e) => {
                    tracing::warn!(path = %path.display(), error = %e, "card image decode failed");
                    return;
                }
            },
            Err(e) => {
                tracing::warn!(path = %path.display(), error = %e, "card image open failed");
                return;
            }
        };
    ...
    pub(super) fn refresh_card_images(&mut self) {
        let ids: Vec<(String, Option<std::path::PathBuf>)> = self
            .agent_cards
            .iter()
        ...
        for (id, path) in ids {
            if self.card_images.contains_key(&id) {
                continue;
            }
            if let Some(path) = path {
    ...
    pub(super) async fn preload_agent_expressions(&mut self) {
        ...
        let id = self
            .agent_id_by_name(&name)
            .or_else(|| self.agent_id_by_name(&self.agent_pref));
        let Some(id) = id else { return };
        let Some(assets_dir) = Self::agent_assets_dir(&id) else {
            return;
        };
