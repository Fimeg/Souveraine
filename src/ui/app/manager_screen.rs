...
use super::{short_id, AgentCard, App};
...
impl App {
    ...
    pub(super) async fn fetch_agent_cards(cfg: ConsciousnessConfig) -> Vec<AgentCard> {
        ...
        let Ok(local) = crate::backend::LocalBackend::new(cfg).await else {
            return vec![];
        };
        let Ok(list) = local.list_agents().await else {
            return vec![];
        };
    ...
    pub(super) fn draw_agent_cards_mut(&mut self, frame: &mut Frame) {
        ...
        self.manager_selected = self
            .manager_selected
            .min(self.agent_cards.len().saturating_sub(1));
        let avail = area.width.saturating_sub((cols + 1) * pad_x);
        ...
        for p in plans {
            ...
            let (cr, cg, cb) = match palette.bg {
                Color::Rgb(r, g, b) => (r, g, b),
                _ => (16, 18, 28),
            };
            let card_bg = Block::default().style(Style::default().bg(Color::Rgb(
                cr.saturating_add(6),
                cg.saturating_add(6),
                cb.saturating_add(6),
            )));
            ...
            let badge_para = Paragraph::new(Line::from(vec![Span::styled(
                badge_text,
                Style::default()
                    .fg(badge_fg)
                    .bg(palette.bg)
            ...
            let meta_lines = vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(instance_label, Style::default().fg(palette.agent_dim)),
                ]),
                Line::from(vec![
                    Span::styled(
                        format!(" {} ", p.name),
                ...
                Line::from(vec![
                    Span::styled(" Files  ", Style::default().fg(palette.agent_dim)),
                    Span::styled(
                        format!("{:<5}", p.memory_count),
                        Style::default().fg(Color::White),
                    ),
                    Span::styled("Uptime  ", Style::default().fg(palette.agent_dim)),
                    Span::styled(
                        format!("{}%", p.uptime_pct),
                ...
                Line::from(vec![
                    Span::styled(" key ", Style::default().fg(palette.agent_dim)),
                    Span::styled(
                        p.pubkey.chars().take(12).collect::<String>(),
                        Style::default().fg(palette.agent_dim),
                    ),
                ]),
            ];
