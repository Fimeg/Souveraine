...
impl App {
    pub(super) async fn init_voice_session(&mut self) {
        ...
        self.voice_client =
            crate::core::voice::VoiceClient::new(&vcfg.stt_url, &vcfg.tts_url, &vcfg.voice_id).ok();
    ...
    pub(super) fn start_listening(&mut self) {
        ...
        match crate::ui::voice::MicCapture::start(16_000) {
            Ok(cap) => {
                ...
                self.presence
                    .set_posture(crate::ui::presence::Posture::Listening);
    ...
    pub(super) async fn handle_presence_space_release(&mut self) {
        ...
        self.presence
            .set_posture(crate::ui::presence::Posture::Thinking);
        ...
        if samples.is_empty() {
            ...
            self.presence
                .set_posture(crate::ui::presence::Posture::Processing);
            return;
        }
    ...
    pub(super) async fn advance_voice_pipeline(&mut self) {
        if let Some(rx) = self.voice_stt_rx.as_mut() {
            let result = match rx.try_recv() {
                ...
                Err(tokio::sync::oneshot::error::TryRecvError::Closed) => {
                    ...
                    self.presence
                        .set_posture(crate::ui::presence::Posture::Straining);
                    return;
                }
        ...
        if let Some(rx) = self.voice_tts_rx.as_mut() {
            let result = match rx.try_recv() {
                ...
                Err(tokio::sync::oneshot::error::TryRecvError::Closed) => {
                    ...
                    self.presence
                        .set_posture(crate::ui::presence::Posture::Idle);
                    return;
                }
                Err(tokio::sync::oneshot::error::TryRecvError::Empty) => {
                    return;
            ...
            match result {
                Ok(mp3_bytes) => {
                    tracing::info!(
                        bytes = mp3_bytes.len(),
                        "TTS bytes received — attempting playback"
                    );
                    ...
                    self.presence
                        .set_posture(crate::ui::presence::Posture::Speaking);
                    if let Some(player) = &self.voice_player {
                        ...
                        tracing::warn!(
                            "TTS bytes ready but no VoicePlayer — audio device unavailable"
                        );
                        self.presence
                            .set_posture(crate::ui::presence::Posture::Idle);
                    }
        ...
        if self.voice_tts_rx.is_none()
            ...
            && !matches!(
                ...
                crate::ui::presence::Posture::Listening | crate::ui::presence::Posture::Speaking
            )
        {
            ...
            let maybe_reply = regen_text.or_else(|| {
                self.chat.as_ref().and_then(|c| {
                    if !c.busy {
                        c.messages.iter().rev().find_map(|m| match m {
                            crate::ui::chat::ChatMessage::Assistant {
                                text,
                                streaming: false,
                                ..
            ...
            if let Some(reply) = maybe_reply {
                ...
                if !already_synthesized {
                    ...
                    tokio::spawn(async move {
                        let result =
                            match crate::core::voice::VoiceClient::new("", &tts_url, &voice) {
                                Ok(c) => c
                                    .synthesize(&reply_for_bytes)
                                    .await
