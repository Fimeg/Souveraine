...
impl App {
    ...
    pub(super) fn build_menu_list(
        &self,
        title: &str,
        palette: &crate::ui::chat::ChatPalette,
    ) -> List<'static> {
    ...
    pub(super) fn render_stat_cards(&self, frame: &mut Frame, area: Rect) {
        ...
        let agents_card = Paragraph::new(format!(
            ...
            if self.agent_status.agent_count == 1 {
                ""
            } else {
                "s"
            },
    ...
    pub(super) fn render_card_image_cover(
        ...
        if !self.cover_protocols.contains_key(&key) {
            ...
            let scaled =
                raw.resize_exact(scaled_w, scaled_h, image::imageops::FilterType::Lanczos3);

            let cropped =
                scaled.crop_imm(0, 0, target_px_w.min(scaled_w), target_px_h.min(scaled_h));

            let proto = picker.new_resize_protocol(cropped);
            self.cover_protocols
                .insert(key.clone(), (area.width, area.height, proto));
        }
    ...
    pub(super) fn render_portrait_card(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let glyph = if self.presence.subconscious_active {
            "◈"
        } else {
            "·"
        };
    ...
    fn draw_welcome_wide(&mut self, frame: &mut Frame) {
        ...
        let (tr, tg, _tb) = match palette.agent_primary {
        ...
        let footer =
            Paragraph::new("↑↓ Navigate • Enter select • a Add • i Inspect • p Presence • q Quit")
                .style(Style::default().fg(palette.agent_dim))
                .alignment(Alignment::Center);
    ...
    fn draw_welcome_stacked(&mut self, frame: &mut Frame) {
        ...
        let footer = Paragraph::new("↑↓ • Enter • a Add • i Inspect • p Presence • q Quit")
            .style(Style::default().fg(palette.agent_dim))
            .alignment(Alignment::Center);
