...
use super::{ChatMessage, ChatMode, ChatState, ImageAttachment, TurnPhase};
...
Tab toggles the cockpit pane. `t` (on empty input) toggles tool expansion.";
    ...
    pub fn spawn_turn(&mut self, text: String) {
        ...
        tokio::spawn(async move {
            match backend
                .send_with_signals(&conv_id, &text, cancel, interject_queue)
                .await
            {
                Ok(mut stream) => {
    ...
    pub fn spawn_turn_with_images(&mut self, text: String, images: Vec<ImageAttachment>) {
        ...
        tokio::spawn(async move {
            match backend
                .send_with_signals_and_images(&conv_id, &text, images, cancel, interject_queue)
                .await
            {
                Ok(mut stream) => {
                    while let Some(ev) = stream.next().await {
    ...
    fn handle_slash_command(&mut self, input: &str) -> bool {
        ...
        if trimmed == "/outfit" || trimmed.starts_with("/outfit ") {
            ...
            self.pending_consciousness
                .push(BackendEvent::Outfit(name.clone()));
            if name.is_empty() {
        ...
        tokio::spawn(async move {
            let result = match ConsciousnessConfig::load(&cfg_path) {
                Ok(cfg) => {
                    ...
                    for m in &all_models {
                        let marker = if bifrost_models.contains(m) {
                            "⚡"
                        } else {
                            "⚙"
                        };
