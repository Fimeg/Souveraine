...
impl ChatPalette {
    ...
    pub fn from_colors(primary: Color, secondary: Color, dim: Color, bg: Color) -> Self {
        let (pr, pg, pb) = match primary {
            Color::Rgb(r, g, b) => (r, g, b),
            _ => (255, 140, 66),
        };
        let (sr, sg, sb) = match secondary {
            Color::Rgb(r, g, b) => (r, g, b),
            _ => (180, 120, 80),
        };
        Self {
...
pub const SLASH_COMMANDS: &[SlashDef] = &[
    ...
    SlashDef {
        name: "/resume",
        hint: "List / switch conversations",
    },
    SlashDef {
        name: "/convos",
    ...
    SlashDef {
        name: "/model",
        hint: "List or set model",
    },
    SlashDef {
        name: "/btw",
        hint: "Interject — deliver text mid-turn",
    },
    SlashDef {
        name: "/code",
        hint: "Shift to code posture (tools expanded, ≡ prompt)",
    },
    SlashDef {
        name: "/chat",
        hint: "Shift to conversation posture (tools collapsed)",
    },
    SlashDef {
        name: "/outfit",
        hint: "Change agent appearance (outfit name)",
    },
    SlashDef {
        name: "/attach",
        hint: "Attach an image file",
    },
];
...
pub struct ChatState {
    ...
    pub switch_rx:
        Option<oneshot::Receiver<Result<(String, Vec<crate::core::session::ConversationMessage>)>>>,
    ...
    pub async fn connect(
        ...
        let human_name = {
            ...
            let p = std::path::PathBuf::from(home)
                ...
                .join(&agent.id)
                .join("memory");
            crate::core::tools::nickname::read_human_name(&p).unwrap_or_else(|| "you".to_string())
        };
