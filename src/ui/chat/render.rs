...
use super::{
    BtwState, ChatMessage, ChatMode, ChatPalette, ChatState, MarkdownCache, MsgLayout,
    ToolResultBlock, TurnPhase, SPINNER,
};
...
pub fn draw(f: &mut Frame, state: &ChatState) {
    ...
    let subconscious_stream_lines = if state.phase == TurnPhase::Subconscious {
        ...
        let live = if state.subconscious_current.is_empty() {
            0
        } else {
            1
        };
    ...
    {
        1 + subconscious_stream_lines
    } else {
        0
    };

    let itinerary_height: u16 = if state.itinerary_line.is_empty() {
        0
    } else {
        1
    };
    ...
    if phase_height > 0 {
        if state.phase == TurnPhase::Subconscious && subconscious_stream_lines > 0 {
            let phase_rows = Layout::default()
                .direction(Direction::Vertical)
                .constraints([Constraint::Length(1), Constraint::Length(phase_height - 1)])
                .split(vchunks[2]);
...
fn draw_phase(f: &mut Frame, state: &ChatState, area: Rect) {
    ...
    let (glyph, label, color) = match state.phase {
        TurnPhase::Thinking | TurnPhase::Idle => {
            (spinner, "Thinking".to_string(), state.palette.agent_primary)
        }
        TurnPhase::Tool => {
...
fn draw_itinerary(f: &mut Frame, state: &ChatState, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("  ⟡ ", Style::default().fg(state.palette.surfacing)),
            Span::styled(&state.itinerary_line, Style::default().fg(glyph_color)),
        ]))
...
fn fade_streaming_tail(mut lines: Vec<Line<'static>>, palette: &ChatPalette) -> Vec<Line<'static>> {
    ...
    for (offset, &amount) in FADE.iter().enumerate() {
        ...
        let blend = |x: u8, t: u8| {
            (x as f32 + (t as f32 - x as f32) * amount)
                .round()
                .clamp(0.0, 255.0) as u8
        };
...
fn draw_messages(f: &mut Frame, state: &ChatState, area: Rect) {
            ...
            ChatMessage::Assistant {
                ...
                let body_lines = if text.is_empty() && *streaming {
                    ...
                    if let Some(c) = &*cached {
                        if c.text_len == key_len
                            && c.inner_width == inner_width
                            && c.palette_hash == palette_hash
                        {
                            c.lines.clone()
                        } else {
                            drop(cached);
                            let lines = markdown::render_with_width(
                                text,
                                agent_color,
                                Some(inner_width),
                                &mdpal,
    ...
    *state.msg_layout.borrow_mut() = MsgLayout {
    ...
    let para = Paragraph::new(lines).scroll((offset, 0)).block(
        Block::default()
...
fn render_tool_card(
    ...
    let (args_lines, rendered_body) = super::tool_renderers::render_card_body(
        name,
        arguments,
...
fn render_tool_card_compact(
    ...
    let arg_budget = (container_width as usize)
        ...
        .clamp(20, 120);
    let args_summary = clip(
        &super::tool_renderers::summarize_tool_args(name, arguments),
        arg_budget,
    );
    ...
    if let Some(r) = result {
        if let Some(detail) =
            super::tool_renderers::compact_result_details(name, &r.output, r.is_error)
        {
            let inner = (container_width as usize).saturating_sub(8).max(20);
            let preview = clip(&detail, inner);
            let detail_color = if r.is_error {
                palette.compaction
            } else {
                palette.tool_dim
            };
...
fn draw_input(f: &mut Frame, state: &ChatState, area: Rect) {
    ...
    for (li, ll) in logical.iter().enumerate() {
        ...
        cursor_byte_remaining = cursor_byte_remaining.saturating_sub(line_len + 1);
        // +1 for \n
    }
