...
fn summarize_edit(map: &serde_json::Map<String, Value>) -> String {
    let path = json_str(map, "path").map(|p| clip(p, 40)).unwrap_or_default();
    let old = json_str(map, "old_string").map(|s| clip(s, 30)).unwrap_or_default();
...
fn summarize_grep(map: &serde_json::Map<String, Value>) -> String {
    let pattern = json_str(map, "pattern").map(|p| clip(p, 40)).unwrap_or_default();
...
fn summarize_glob(map: &serde_json::Map<String, Value>) -> String {
    let pattern = json_str(map, "pattern").map(|p| clip(p, 50)).unwrap_or_default();
    if pattern.is_empty() {
        return summarize_generic(map);
    }
...
fn summarize_subagent(map: &serde_json::Map<String, Value>) -> String {
    let prompt = json_str(map, "prompt").map(|s| clip(s, 50)).unwrap_or_default();
...
fn summarize_summon(map: &serde_json::Map<String, Value>) -> String {
    let target = json_str(map, "target").map(|t| clip(t, 40)).unwrap_or_default();
    if target.is_empty() {
        return summarize_generic(map);
    }
    target
...
pub fn render_card_body(
    ...
    let args_line = Line::from(vec![Span::styled(args_summary, Style::default().fg(dim_color))]);
    ...
    let body_lines = match (name, output) {
        ...
        ("read", Some(out)) if !out.is_empty() => render_read_body(out, arguments, inner_width, palette, &mdpal),
        ("grep", Some(out)) if !out.is_empty() => render_grep_body(out, inner_width, palette, &mdpal),
        ("glob", Some(out)) if !out.is_empty() => render_glob_body(out, inner_width, palette, &mdpal),
        ("write", Some(out)) if !out.is_empty() => render_write_body(out, arguments, inner_width, palette),
        ("edit", Some(out)) if !out.is_empty() => render_edit_body(out, arguments, inner_width, palette),
        ("memory", Some(out)) => render_memory_body(out, inner_width, &mdpal, palette),
        ("list_dir", Some(out)) if !out.is_empty() => render_list_dir_body(out, inner_width, palette),
        _ => {
...
fn render_bash_body(
    ...
    lines.push(Line::from(Span::styled(bottom, Style::default().fg(dim).add_modifier(Modifier::DIM))));
...
fn render_read_body(
    ...
    let dim_style = Style::default().fg(mdpal.code_fg).add_modifier(Modifier::DIM);
...
fn render_grep_body(
    ...
    if let Some(first) = output.lines().next() {
        lines.push(Line::from(Span::styled(
            ...
            Style::default().fg(palette.tool_accent).add_modifier(Modifier::BOLD),
        )));
        lines.push(Line::from(Span::styled(
...
fn render_memory_body(
    ...
    if !body.is_empty() {
        let max_lines = 30usize;
        let preview: String = body.lines().take(max_lines).collect::<Vec<&str>>().join("\n");
        let rendered = crate::ui::markdown::render_with_width(
            &preview,
            mdpal.code_fg,
...
fn summarize_frontmatter(frontmatter: &str) -> String {
    ...
    if !tags.is_empty() { parts.push(&tags); }
    if !limit.is_empty() { parts.push(&limit); }
    if parts.is_empty() {
...
fn arguments_to_path(arguments: &str) -> String {
    ...
    match parsed {
        Ok(Value::Object(map)) => {
            json_str(&map, "path").map(|s| s.to_string()).unwrap_or_default()
        }
        _ => String::new(),
    }
}
