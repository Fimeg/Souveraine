    ...
    fn build_lines(&self) -> Vec<Line<'static>> {
        ...
        lines.push(self.row(
            ...
            vec![Span::styled(
                ...
                Style::default().fg(if self.n1_active {
                    p.surfacing
                } else {
                    p.agent_primary
                }),
        ...
        lines.push(self.row(
            "◈ surf",
            vec![Span::styled(
                format!("{} surfaced", self.surfacings),
                Style::default().fg(p.surfacing),
            )],
        ));
        ...
        lines.push(self.row(
            "◎ N+25",
            vec![match &self.last_reflection {
        ...
        lines.push(self.row(
            "⚠ compact",
            vec![match &self.last_compaction {
                Some((tier, t)) => {
        ...
        let strain_spans: Vec<Span> = if self.strain_504 + self.strain_429 + self.strain_other == 0
        {
            ...
            if self.strain_429 > 0 {
                ...
                v.push(Span::styled(
                    format!("429×{}", self.strain_429),
                    Style::default().fg(p.compaction),
                ));
            }
        ...
        lines.push(self.row(
            ...
            vec![
                Span::styled(
                    Self::bar(self.energy as f32 / 100.0, 5),
                    Style::default().fg(p.agent_primary),
                ),
                Span::styled(
                    format!("  {}", self.energy),
