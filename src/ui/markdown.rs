...
impl MarkdownPalette {
    pub fn from_chat_palette(cpal: &ChatPalette) -> Self {
        ...
        Self {
            code_bg: Color::Rgb(
                r(bg).saturating_add(22),
                g(bg).saturating_add(22),
                b(bg).saturating_add(34),
            ),
...
pub fn wrap_line_with_gutter(
    ...
    let seed_gutter =
        |current: &mut Vec<Span<'static>>, current_width: &mut usize, pending: &mut bool| {
            if *pending {
...
fn balanced_breaks(
    ...
    for start in (0..n).rev() {
        ...
        for end in start..n {
            ...
            let better = cost < dp[start] || (cost == dp[start] && lines_used < counts[start]);
            if better {
                dp[start] = cost;
                breaks[start] = end + 1;
    ...
    fn ensure_line_started(&mut self) {
        if self.current.is_empty() {
            ...
            if let Some(b) = self.pending_bullet.take() {
                self.current
                    .push(Span::styled(b, Style::default().fg(self.palette.bullet)));
            }
        }
    }
