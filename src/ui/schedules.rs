    ...
    if view.entries.is_empty() {
        ...
        for (idx, entry) in view.entries.iter().enumerate() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!("{:<24}", entry.schedule),
                    Style::default().fg(p.agent_primary),
                ),
                Span::styled(
                    format!("urg {:.1}", entry.urgency),
                    Style::default().fg(p.agent_dim),
                ),
            ]));
            if selected {
...
fn draw_create_form(frame: &mut Frame, area: Rect, form: &CreateForm, palette: &ChatPalette) {
    ...
    let is_field =
        |f: CreateField| std::mem::discriminant(&form.field) == std::mem::discriminant(&f);
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            "    (seconds for interval, cron expr for cron)",
            Style::default().fg(palette.agent_dim),
        ),
    ]));
    lines.push(Line::from(vec![
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:.1}", form.urgency),
            field_style(CreateField::Urgency),
        ),
        Span::styled(
            "    (←/→ to adjust, 0.0–1.0)",
            Style::default().fg(palette.agent_dim),
...
fn footer_for(mode: &Mode) -> String {
    match mode {
        Mode::Browse => {
            "j/k navigate • c create • e toggle • r run-now • d delete • q back".to_string()
        }
