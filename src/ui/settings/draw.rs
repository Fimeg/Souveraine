...
use super::types::{Category, PanelFocus, SettingsMode};
...
fn draw_field_panel(frame: &mut Frame, area: Rect, view: &SettingsView) {
    ...
    for (i, (loc, value)) in fields.iter().enumerate() {
        ...
        let mut spans = vec![Span::styled(format!("  {:25}", loc.label()), key_style)];

        // If this field is being edited, show the buffer with cursor.
        if !is_readonly {
            if let SettingsMode::Editing {
                loc: edit_loc,
                buffer,
                cursor,
            } = &view.mode
    ...
    let border_color = if view.dirty {
        let (r, g, b) = match p.agent_primary {
...
fn draw_footer(frame: &mut Frame, area: Rect, view: &SettingsView) {
    let msg: String = match &view.mode {
        SettingsMode::Browse => {
            ...
            if matches!(view.selected_category(), Category::Providers) {
                if view.models_fetching {
                    ...
                    format!(
                        "{base} · [r] refresh  {} models",
                        view.available_models.len()
                    )
                }
            } else {
                base.to_string()
        ...
        SettingsMode::Editing { .. } => {
            " Enter confirm · Tab confirm+next · Esc cancel".to_string()
        }
        SettingsMode::ConfirmDiscard => {
            " Unsaved changes — y discard & quit · Esc cancel".to_string()
        }
    ...
    let footer =
        Paragraph::new(format!(" {msg}")).style(Style::default().fg(view.palette.agent_dim));
    frame.render_widget(footer, area);
}

fn draw_saved(frame: &mut Frame, area: Rect, msg: &str) {
    let text =
        Paragraph::new(format!(" ✓ {msg}")).style(Style::default().fg(Color::Rgb(120, 200, 120)));
    frame.render_widget(text, area);
}

fn draw_error(frame: &mut Frame, area: Rect, msg: &str) {
    let text =
        Paragraph::new(format!(" ✗ {msg}")).style(Style::default().fg(Color::Rgb(220, 100, 100)));
    frame.render_widget(text, area);
}
