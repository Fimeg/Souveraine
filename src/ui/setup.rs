...
impl SetupState {
    pub fn new(flow: SetupFlow, default_model: &str) -> Self {
        ...
        // Souveraine, called Souvie, is the one default agent and `souvie` the
        // one default account. Import and federation carry their own identity;
        // proposing a name there writes one fleet's agents into the product.
        let (agent_name, account_name) = match flow {
    ...
    fn draw_form(&self, frame: &mut Frame, area: Rect, title: &str) {
        ...
        for (i, slot) in self.form.slots.iter().enumerate() {
            ...
            let val_style = if dv.is_empty() {
                Style::default().fg(Color::Rgb(80, 80, 80))
            } else {
                Style::default().fg(Color::White)
            };
            lines.push(Line::from(Span::styled(
                format!(" {}{}", dv, cursor_marker),
                val_style,
            )));
            lines.push(Line::from(""));
        }
    ...
    fn draw_import_screen(&self, frame: &mut Frame, area: Rect) {
        let mut lines: Vec<Line> = vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Your agent is created. Now you can:",
                Style::default().fg(Color::Gray),
            )),
            Line::from(""),
        ];
        ...
        if !self.letta_agents.is_empty() {
            ...
            lines.push(Line::from(Span::styled(
                "  \u{2B21}  Import from Letta  (no Letta agents found)",
                Style::default().fg(Color::Rgb(80, 80, 80)),
            )));
            lines.push(Line::from(""));
        }
        ...
        lines.push(Line::from(Span::styled(
            "  \u{2B21}  Sync from federation peer",
            Style::default().fg(Color::Rgb(150, 200, 255)),
        )));
        lines.push(Line::from(Span::styled(
            "       (federation transport not yet built \u{2014} coming soon)",
            Style::default()
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            "  \u{2B21}  Skip \u{2014} I'll set it up later",
    ...
    fn draw_complete(&self, frame: &mut Frame, area: Rect) {
        let mut lines = vec![
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Your agent is ready. Here's what was configured:",
                Style::default().fg(Color::Gray),
            )),
            Line::from(""),
        ];
        ...
        lines.push(Line::from(Span::styled(
            format!(
                ...
                if has_key {
                    ...
                    "no key \u{2014} local fallback"
                }
            ),
            Style::default().fg(Color::Rgb(150, 150, 150)),
        )));
        lines.push(Line::from(Span::styled(
            format!("  Model:       {}", self.model_handle),
            Style::default().fg(Color::Rgb(150, 200, 255)),
        )));
        ...
        if let Some(ref import) = self.import_selection {
            match import {
                ImportOption::Letta { id, name } => {
                    lines.push(Line::from(Span::styled(
                        format!("  Imported:    {} from Letta ({})", name, id),
                        Style::default().fg(Color::Rgb(150, 200, 150)),
                    )));
                }
                ImportOption::Federation => {
                    lines.push(Line::from(Span::styled(
                        "  Federation:  peer sync configured",
                        Style::default().fg(Color::Rgb(150, 200, 255)),
                    )));
                }
                ImportOption::Skip => {}
