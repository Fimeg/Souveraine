...
DelegateChooser {
    ...
    // Phone-only roles: inert unless the toggle arrangement includes them
    // (laptop configs never do), so they live in the shared base.
    ...
    DelegateChoice { roleValue: "wallpaperShuffle"; AndroidWallpaperShuffleToggle {
        required property int index
        required property var modelData
        buttonIndex: root.startingIndex + index
        buttonData: modelData
